...
def search_place():
    ...
    if len(rez) == 0:
        ...
        for r in rez:
            ...
            owc = []
            for item in data:
                pr = pr+int(item["quantity"]) * (db.session.query(Menu_str.cost).filter(
                    Menu_str.dish_id == int(item["dish"])).filter(Menu_str.restaurant_id == int(r[0])).first()[0])
                owp = (db.session.query(Menu_str.cost).filter(Menu_str.dish_id == int(
                    item["dish"])).filter(Menu_str.restaurant_id == int(r[0])).first()[0])
                own = db.session.query(Dish.name).filter(
                    Dish.id == int(item["dish"])).first()[0]
...
def cart():
    ...
    for item in data:
        cart_dish_id = item["dish"]
        dishes = query.filter(Dish.id == int(cart_dish_id)).all()
        dish_cart_list.append(
            {"dish": dishes[0], "quantity": item["quantity"]})
